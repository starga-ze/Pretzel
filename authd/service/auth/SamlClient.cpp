...
bool SamlClient::ensureXmlSecInit()
{
    static bool ok = []
    {
        ...
        if (xmlSecInit() < 0)
        {
            LOG_ERROR("xmlsec init failed");
            return false;
        }
        if (xmlSecCryptoAppInit(nullptr) < 0)
        {
            LOG_ERROR("xmlsec crypto app init failed");
            return false;
        }
        if (xmlSecCryptoInit() < 0)
        {
            LOG_ERROR("xmlsec crypto init failed");
            return false;
        }
        return true;
    }();
...
SamlClient::StartResult SamlClient::buildAuthnRedirectUrl(const std::string& relayState)
{
    ...
    if (!rawDeflate(xml, deflated))
    {
        r.error = "deflate failed";
        return r;
    }
...
bool SamlClient::verifySignature(void* docV, void*& signedAssertion, std::string& errOut) const
{
    ...
    bool ok = false;
    if (xmlSecCryptoAppKeysMngrCertLoadMemory(mngr, reinterpret_cast<const xmlSecByte*>(m_cfg.idpCertPem.data()),
                                              static_cast<xmlSecSize>(m_cfg.idpCertPem.size()), xmlSecKeyDataFormatPem,
...
SamlClient::Result SamlClient::verifyResponse(const std::string& base64SamlResponse)
{
    ...
    xmlDocPtr doc = xmlReadMemory(reinterpret_cast<const char*>(xml.data()), static_cast<int>(xml.size()), "saml.xml",
                                  nullptr, XML_PARSE_NONET | XML_PARSE_NOENT);
    if (!doc)
    {
        r.error = "xml parse failed";
        return r;
    }
    ...
    if (xmlNodePtr cond = childByLocal(assertion, "Conditions"))
    {
        ...
        if (xmlNodePtr ar = childByLocal(cond, "AudienceRestriction"))
            for (xmlNodePtr a = ar->children; a; a = a->next)
                if (localIs(a, "Audience") && nodeText(a) == m_cfg.spEntityId)
                    audOk = true;
        if (!audOk)
        {
            xmlFreeDoc(doc);
            r.error = "audience mismatch";
            return r;
        }
    }
