...
std::unique_ptr<EnginedEvent> ScanService::schedule(std::chrono::steady_clock::time_point now)
{
    if (m_pending)
    {
...
void ScanService::handleEvent(EnginedServiceManager& serviceManager, const ScanEvent& event)
{
    switch (event.type())
    {
    ...
    case ScanEventType::ReceiveScanResult:
    {
        ...
        persistDevices(serviceManager, std::string(reinterpret_cast<const char*>(pl.data()), pl.size()));
        break;
    }
...
void ScanService::sendScanRequest(EnginedServiceManager& serviceManager)
{
    ...
    pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(
        pz::ipc::IpcDaemon::Engined, pz::ipc::IpcDaemon::Scand, pz::ipc::IpcCmd::ScanRequest,
        static_cast<std::uint32_t>(json.size()), pz::ipc::IpcProtocol::toFlag(pz::ipc::IpcFlag::Request));
...
void ScanService::persistDevices(EnginedServiceManager& serviceManager, const std::string& payloadJson)
{
    nlohmann::json root;
    try
    {
    ...
    for (const auto& d : devices)
    {
        ...
        const std::string macs = d.value("interface_macs", nlohmann::json::array()).dump();
        const std::string ifaces = d.value("interfaces", nlohmann::json::array()).dump();
        const std::string ifTable = d.value("if_table", nlohmann::json::array()).dump();
        const std::string neighbors = d.value("lldp_neighbors", nlohmann::json::array()).dump();
        const std::string arp = d.value("arp_entries", nlohmann::json::array()).dump();

        const std::string sysObjectId = d.value("sys_object_id", "");
        const std::string vendor = serviceManager.vendorResolver().vendorForSysObjectId(sysObjectId);

        const bool ok =
            db.exec("INSERT INTO probe_devices "
                    ...
                    {ip, d.value("sys_name", ""), d.value("sys_descr", ""), sysObjectId, d.value("sys_contact", ""),
                     d.value("sys_location", ""), std::to_string(d.value("sys_up_time_ticks", 0u)), macs, ifaces,
                     ifTable, neighbors, arp, vendor});
        if (ok)
            ++written;
    }
