...
void HeartbeatService::handleEvent(TopologydServiceManager& serviceManager, const HeartbeatEvent& event)
{
    switch (event.type())
    {
...
void HeartbeatService::handleAction(TopologydServiceManager& serviceManager, const HeartbeatAction& action)
{
    switch (action.type())
    {
    case HeartbeatActionType::SendHeartbeatResponse:
    {
        ...
        pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Topologyd, action.dst(),
