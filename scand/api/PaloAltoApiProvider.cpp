...
// "192.168.1.1/23" -> ip="192.168.1.1", prefix=23. Returns false for "N/A"/empty.
...
// PAN-OS speed field is "1000" (Mbps), "[n/a]", "ukn", or "". Non-numeric -> 0.
...
// Parse the XML, verify <response status="success">, hand the <result> subtree to
// `fn`. Returns false on parse error or non-success status.
template <typename Fn>
bool withResult(const std::string& xml, Fn&& fn)
{
    try
    {
        pt::ptree         root;
...
std::string PaloAltoApiProvider::resolveApiKey(const ApiCredential& cred,
                                               const std::string& host)
{
    ...
    const std::string body =
        "type=keygen&user=" + HttpsClient::urlEncode(cred.username) +
        "&password=" + HttpsClient::urlEncode(cred.password);

    const auto resp = HttpsClient::postForm(host, cred.port, "/api/", body,
    ...
    std::string key;
    withResult(resp.body, [&](const pt::ptree& result) {
        key = result.get<std::string>("key", "");
    });
    return key;
}

std::string PaloAltoApiProvider::op(const ApiCredential& cred, const std::string& host,
                                    const std::string& apiKey,
                                    const std::string& xmlCmd)
{
    const std::string target = "/api/?type=op&cmd=" + HttpsClient::urlEncode(xmlCmd) +
                               "&key=" + HttpsClient::urlEncode(apiKey);
    const auto resp = HttpsClient::get(host, cred.port, target,
                                       cred.timeoutMs, cred.verifyTls);
    if (!resp.ok)
    {
...
void PaloAltoApiProvider::parseInterfaces(const std::string& xml, SnmpDevice& dev)
{
    ...
    withResult(xml, [&](const pt::ptree& result) {
        // hw: physical/logical interface inventory (name, id, mac, speed, state).
        ...
        // ifnet: logical interfaces carrying IPs (the topology-relevant set).
        if (const auto ifnet = result.get_child_optional("ifnet"))
        {
            for (const auto& kv : *ifnet)
            {
                ...
                addIp(e.get<std::string>("ip", ""));            // primary
                if (const auto addr = e.get_child_optional("addr"))
                    for (const auto& m : *addr)                 // secondaries
                        if (m.first == "member")
    ...
    // API is authoritative for PAN-OS interface data — replace the (partial) SNMP
    // view only when the API actually returned something.
    if (!ifTable.empty())
    {
        dev.ifTable = std::move(ifTable);
        // Cache the name->ifIndex map on the device's MAC fingerprint? No — ARP/LLDP
        // parse below reuse dev.ifTable directly.
    }
    if (!interfaces.empty())
...
{
    // Build interface-name -> ifIndex from the (already parsed) ifTable.
    std::unordered_map<std::string, uint32_t> nameToIdx;
    ...
    withResult(xml, [&](const pt::ptree& result) {
        ...
        for (const auto& kv : *entries)
        {
            ...
            if (mac.empty() || mac == "(incomplete)")
                continue;   // unresolved ARP — no MAC to learn
    ...
    withResult(xml, [&](const pt::ptree& result) {
        for (const auto& kv : result)
        {
            ...
            const std::string localName =
                portEntry.get<std::string>("<xmlattr>.name", "");
...
bool PaloAltoApiProvider::collect(const ApiCredential& cred, SnmpDevice& dev)
{
    ...
    if (auto xml = op(cred, host, apiKey,
                      "<show><interface>all</interface></show>"); !xml.empty())
        parseInterfaces(xml, dev);

    if (auto xml = op(cred, host, apiKey,
                      "<show><arp><entry name=\"all\"/></arp></show>"); !xml.empty())
        parseArp(xml, dev);

    if (auto xml = op(cred, host, apiKey,
                      "<show><lldp><neighbors>all</neighbors></lldp></show>"); !xml.empty())
        parseLldp(xml, dev);

    if (dev.sysName.empty() || dev.sysDescr.empty())
        if (auto xml = op(cred, host, apiKey,
                          "<show><system><info></info></system></show>"); !xml.empty())
            parseSystemInfo(xml, dev);

    const bool gained = dev.interfaces.size() != beforeIf ||
