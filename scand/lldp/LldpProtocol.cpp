...
std::vector<LldpNeighbor> LldpProtocol::walkNeighbors(void* sessp)
{
    ...
    walkColumn(sessp, kLldpLocPortDesc, kLldpLocColLen,
    ...
    {
        if (var->name_length < kLldpRemColLen + 3)
            return {0, 0};
        return {static_cast<uint32_t>(var->name[kLldpRemColLen + 1]),
                static_cast<uint32_t>(var->name[kLldpRemColLen + 2])};
    };

    walkColumn(sessp, kLldpRemSysName, kLldpRemColLen,
               [&](netsnmp_variable_list* var) { byKey[keyOf(var)].remoteSysName = octetDisplay(var); });
    walkColumn(sessp, kLldpRemSysDesc, kLldpRemColLen,
               [&](netsnmp_variable_list* var) { byKey[keyOf(var)].remoteSysDescr = octetDisplay(var); });
    walkColumn(sessp, kLldpRemPortId, kLldpRemColLen,
               [&](netsnmp_variable_list* var) { byKey[keyOf(var)].remotePortId = octetDisplay(var); });
    walkColumn(sessp, kLldpRemChassisId, kLldpRemColLen,
               [&](netsnmp_variable_list* var) { byKey[keyOf(var)].remoteChassisId = octetDisplay(var); });
    walkColumn(sessp, kLldpRemChassisIdSubtype, kLldpRemColLen,
               [&](netsnmp_variable_list* var)
               { byKey[keyOf(var)].remoteChassisIdSubtype = static_cast<int>(varInt(var)); });
    walkColumn(sessp, kLldpRemSysCapEnabled, kLldpRemColLen,
               [&](netsnmp_variable_list* var)
               {
