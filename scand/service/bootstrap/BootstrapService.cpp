...
BootstrapService::BootstrapService(ScandEventFactory* eventFactory, ScandActionFactory* actionFactory)
...
std::unique_ptr<ScandEvent> BootstrapService::schedule(std::chrono::steady_clock::time_point now)
{
    if (!m_eventFactory)
    {
    ...
    case State::Init:
    {
        ...
        return m_eventFactory->create(ScandEventDomain::Bootstrap,
...
void BootstrapService::handleEvent(ScandServiceManager& serviceManager, const BootstrapEvent& event)
{
    if (!m_actionFactory)
    {
    ...
    {
        auto action = m_actionFactory->create(ScandActionDomain::Bootstrap,
...
void BootstrapService::handleAction(ScandServiceManager& serviceManager, const BootstrapAction& action)
{
...
void BootstrapService::onServerHello(ScandServiceManager& serviceManager, const pz::ipc::IpcMessage& msg)
{
    (void)msg;
...
{
    std::string name = pz::ipc::IpcProtocol::daemonToStr(pz::ipc::IpcDaemon::Scand);

    auto flag = pz::ipc::IpcProtocol::toFlag(pz::ipc::IpcFlag::Request);

    pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Scand, pz::ipc::IpcDaemon::Ipcd,
