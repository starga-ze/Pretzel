...
void HeartbeatService::handleEvent(ScandServiceManager& serviceManager, const HeartbeatEvent& event)
{
    switch (event.type())
    {
...
void HeartbeatService::handleAction(ScandServiceManager& serviceManager, const HeartbeatAction& action)
{
    switch (action.type())
    {
    case HeartbeatActionType::SendHeartbeatResponse:
    {
        ...
        pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Scand, action.dst(),
