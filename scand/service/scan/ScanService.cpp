...
void ScanService::handleEvent(ScandServiceManager& sm, const ScanEvent& event)
{
    ...
    default:
        LOG_WARN("unhandled event (type={})", static_cast<uint32_t>(event.type()));
        break;
    }
}
...
void ScanService::handleScanRequest(ScandServiceManager& sm, const ScanEvent& event)
{
    ...
    LOG_INFO("forwarding scan (snmp_ips={}, v3_devices={}, api_devices={})", snmpIps.size(), cfg.v3PerIp.size(),
             apiDevices.size());
...
void ScanService::finalizeIfReady(ScandServiceManager& sm)
{
    ...
    sm.postAction(std::make_unique<ScanAction>(ScanActionType::SendScanResult, root.dump()));
}
...
void ScanService::handleAction(ScandServiceManager& sm, const ScanAction& action)
{
    ...
    {
        LOG_DEBUG("sending ScanResult to engined (len={})", action.resultJson().size());

        const std::string& json = action.resultJson();
        std::vector<uint8_t> payload(json.begin(), json.end());

        auto header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Scand, pz::ipc::IpcDaemon::Engined,
                                                pz::ipc::IpcCmd::ScanResult, static_cast<uint32_t>(payload.size()),
    ...
    default:
        LOG_WARN("unhandled action (type={})", static_cast<uint32_t>(action.type()));
        break;
    }
}
