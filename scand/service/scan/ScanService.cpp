...
void ScanService::handleEvent(ScandServiceManager& sm, const ScanEvent& event)
{
    ...
    case ScanEventType::ReceiveScanRequest:
    {
        ...
        if (!msg)
        {
            LOG_WARN("ScanService: ReceiveScanRequest has empty message");
            return;
        }
        ...
        if (payload.empty())
        {
            LOG_WARN("ScanService: ScanRequest payload is empty");
            return;
        }
        ...
        {
            LOG_WARN("ScanService: failed to parse ScanRequest payload: {}", e.what());
            return;
        }

        if (ips.empty())
        {
            LOG_DEBUG("ScanService: ScanRequest has no IPs — skipping");
            return;
        }
        ...
        // Terminal stage of the v2c -> v3 -> API chain: per-IP vendor-API creds,
        // owned by ApiService (config section scand.service.api.devices[]).
        cfg.apiPerIp = sm.apiService().loadCredentials();

        LOG_INFO("ScanService: forwarding scan to SnmpEngine ips={} v3_devices={} api_devices={}",
                 ips.size(), cfg.v3PerIp.size(), cfg.apiPerIp.size());

        // Delegate to SnmpEngine via TxRouter (non-blocking — engine polls in tick)
        sm.txRouter().startScan(std::move(ips), std::move(cfg));
        break;
    }
...
void ScanService::handleAction(ScandServiceManager& sm, const ScanAction& action)
{
    ...
    {
        LOG_DEBUG("ScanService: sending ScanResult to engined len={}",
