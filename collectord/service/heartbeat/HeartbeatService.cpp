...
void HeartbeatService::handleEvent(CollectordServiceManager& serviceManager, const HeartbeatEvent& event)
{
    switch (event.type())
    {
...
void HeartbeatService::handleAction(CollectordServiceManager& serviceManager, const HeartbeatAction& action)
{
    switch (action.type())
    {
    case HeartbeatActionType::SendHeartbeatResponse:
    {
        ...
        pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Collectord, action.dst(),
