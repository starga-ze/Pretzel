...
void HeartbeatService::handleEvent(ApidServiceManager& serviceManager, const HeartbeatEvent& event)
{
    switch (event.type())
    {
...
void HeartbeatService::handleAction(ApidServiceManager& serviceManager, const HeartbeatAction& action)
{
    switch (action.type())
    {
    case HeartbeatActionType::SendHeartbeatResponse:
    {
        ...
        pz::ipc::IpcHeader header = pz::ipc::IpcHeader::build(pz::ipc::IpcDaemon::Apid, action.dst(),
