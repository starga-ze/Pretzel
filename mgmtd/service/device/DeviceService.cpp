...
bool isGroupingMac(const std::string& mac)
{
    ...
    if (mac == "ff:ff:ff:ff:ff:ff")
        return false;
    if (mac.rfind("00:00:5e:00:01:", 0) == 0)
        return false;
    if (mac.rfind("00:00:0c:07:ac:", 0) == 0)
        return false;
    if (mac.rfind("00:00:0c:9f:f", 0) == 0)
        return false;
    return true;
}
...
void classify(const IpRecord& rep, std::string& type, std::string& subtype)
{
    ...
    if (contains(s, "firewall") || contains(s, "fortigate") || contains(s, "palo alto") || contains(s, "fortios") ||
        contains(s, " asa") || contains(s, "srx") || contains(s, "pan-os"))
    {
        type = "network";
        subtype = "firewall";
        return;
    }
    if (contains(s, "access point") || contains(s, "wireless") || contains(s, "aironet") || contains(s, "unifi") ||
        contains(s, "aruba ap"))
    {
        type = "network";
        subtype = "ap";
    ...
    if (contains(s, "router") || contains(s, "routeros") || contains(s, "mikrotik") || contains(s, " ios ") ||
        contains(s, "cisco ios") || contains(s, "vyos"))
    {
        type = "network";
        subtype = "router";
        return;
    }
    if (contains(s, "switch") || contains(s, "catalyst") || contains(s, "procurve") || contains(s, "nexus") ||
        contains(s, "powerconnect") || contains(s, "powerswitch") || contains(s, "force10") ||
        contains(s, "networking os"))
    {
        type = "network";
        subtype = "switch";
        return;
    }
    if (contains(s, "gateway") || contains(s, "load balancer") || contains(s, "netscaler") || contains(s, "f5 ") ||
        contains(s, "big-ip"))
    {
        type = "network";
        subtype = "gateway";
    ...
    if (contains(s, "vmware") || contains(s, "esxi") || contains(s, "hypervisor") || contains(s, "proxmox") ||
        contains(s, "hyper-v"))
    {
        type = "server";
        subtype = "hypervisor";
    ...
    if (contains(s, "idrac") || contains(s, "ilo") || contains(s, "ipmi") || contains(s, "bmc") ||
        contains(s, "supermicro") || contains(s, "ibmc") || contains(s, "integrated management"))
    {
        type = "server";
        subtype = "bmc";
        return;
    }
    if (contains(s, "windows"))
    {
        type = "server";
        subtype = "windows";
        return;
    }
    if (contains(s, "linux") || contains(s, "ubuntu") || contains(s, "debian") || contains(s, "centos") ||
        contains(s, "red hat") || contains(s, "darwin") || contains(s, "macos"))
    {
        type = "server";
        subtype = "linux";
        return;
...
std::vector<DeviceGroup> DeviceService::groups() const
{
    ...
    for (const auto& row : db.queryRows("SELECT ip, status, mac, host_vendor, sys_name, sys_descr, sys_object_id, "
                                        ...
                                        "snmp_vendor, if_table, lldp_neighbors FROM probe_devices"))
    {
        ...
        if (macsJson.is_array())
        {
            for (const auto& m : macsJson)
            {
                ...
                if (isGroupingMac(mac) && std::find(rec.macs.begin(), rec.macs.end(), mac) == rec.macs.end())
        ...
        rec.hasSnmp = !rec.sysName.empty() || !rec.sysObjectId.empty() || !rec.macs.empty() ||
                      !rec.interfaces.empty() || !rec.ifTable.empty();
    }
    ...
    for (auto& [root, idxs] : members)
    {
        ...
        std::sort(g.interfaces.begin(), g.interfaces.end(),
                  [](const DeviceInterface& a, const DeviceInterface& b) { return ipLess(a.ip, b.ip); });
        ...
        for (int idx : idxs)
        {
            ...
            if ((candSnmp && !curSnmp) || (candSnmp == curSnmp && ipLess(recs[idx]->ip, recs[primaryIdx]->ip)))
                primaryIdx = idx;
        }
        ...
        if (!p.hasSnmp && !g.vendor.empty())
        {
            ...
            if (contains(v, "ruckus") || contains(v, "aruba") || contains(v, "mist") || contains(v, "meraki") ||
                contains(v, "ubiquiti") || contains(v, "aerohive"))
                g.subtype = "ap";
        }
    ...
    std::sort(out.begin(), out.end(),
              [&](const DeviceGroup& a, const DeviceGroup& b)
              {
                  if (a.type != b.type)
