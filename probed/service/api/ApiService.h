...
// One reusable set of vendor API credentials. Authored in the mgmtd UI and stored in
// collectord.service.api.api_credentials — the whole API config domain (credentials, endpoints,
// connectors) lives in one section because a collection connector references a credential by oid,
// so they are referentially inseparable. probed only consumes the credential slice of that section
// (it owns keygen/health/auto-refresh, collectord owns collection), reading it cross-section the
// same way it already reads engined.service.site for a device's target.
//
// Secrets are NOT part of the profile: the issued key is sealed with the credential store and lives
// in engined's api_credential_state, keyed by the profile oid.
//
...
// api_credential_state so probed can re-issue on its own for auto-refresh. Memory-only.
...
// loaded profiles: the operator runs a test on values typed into the UI that have not been
...
// probed's owner of the vendor-API credential lifecycle: it holds the auth profiles and the opened
// secrets, answers connector-test requests (ApiConnectorTester), and drives auto-refresh. collectord
// keeps its own trimmed ApiService for the collection side; the two share nothing but engined's
// api_credential_state, which each requests independently.
class ApiService
{
public:
    ApiService() = default;
    ...
    void handleEvent(ProbedServiceManager& serviceManager, const ApiEvent& event);
    ...
    void requestKeys(ProbedServiceManager& serviceManager);
