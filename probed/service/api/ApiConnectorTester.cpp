...
struct ConnectorTest
{
    ...
    ProbedServiceManager* sm{nullptr};
...
// call order below.
void sendTestResponse(ProbedServiceManager& sm, std::uint32_t seqNo, const json& out);
void sendApiCredentialState(ProbedServiceManager& sm, const std::string& keyOid, const std::string& id,
...
void ApiConnectorTester::run(ApiService& api, ProbedServiceManager& sm, std::uint32_t seqNo, const json& input)
{
